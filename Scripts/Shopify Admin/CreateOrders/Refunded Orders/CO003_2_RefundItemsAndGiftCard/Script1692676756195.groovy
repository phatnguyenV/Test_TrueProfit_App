...
def productItems = ['Welcome','Antique Drawers','Cream Sofa','Pink Armchair']
...
WebUI.sendKeys(input_search_products, Keys.chord(Keys.CONTROL, 'a', Keys.BACK_SPACE) + productItems[0])

WebUI.waitForElementVisible(gift_card_100, 2)

WebUI.click(gift_card_100)

WebUI.sendKeys(input_search_products_has_value, Keys.chord(Keys.CONTROL, 'a', Keys.BACK_SPACE) + productItems[1])

WebUI.waitForElementVisible(select_first_item, 2)
...
WebUI.sendKeys(input_search_products_has_value, Keys.chord(Keys.CONTROL, 'a', Keys.BACK_SPACE) + productItems[3])
...
// Loop only 2 times and sendKeys to the input fields
for (int i = 0; i < 2 && i < textfields.size(); i++) {
	textfields[i].sendKeys(Keys.chord(Keys.CONTROL, "a", Keys.BACK_SPACE) + "2")
}
...
WebUI.waitForElementVisible(refund_gift_card_quantity, 3)

WebUI.sendKeys(refund_gift_card_quantity, '1')
